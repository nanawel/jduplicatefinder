	...
	public void run() {
		try {
			...
			for(DateTime.CacheUnit currentFile : this.fileCache) {
				...
				logger.debug(currentFile.file.getAbsolutePath() + ": " + lowerLimit + " <= date < " + upperLimit);
...
//				}
				...
				for(CacheUnit sf : idxSimilarFiles.retrieve(query)) {
					...
					//logger.debug("\tFound: " + sf.file.getAbsolutePath() + ": " + sf.timestamp);
	...
	public class CacheUnit
	{
		...
		public CacheUnit(File file) throws IOException {
			this.file = file;
			...
			logger.debug(file.getPath() + ": " + this.getTimestamp() + " (" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss z").format(date) + ")");
		}
