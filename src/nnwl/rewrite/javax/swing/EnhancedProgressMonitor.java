package nnwl.rewrite.javax.swing;
...
public class EnhancedProgressMonitor implements Accessible {
	private EnhancedProgressMonitor root;
	 ...
	 * @see JOptionPane
	 */
	public EnhancedProgressMonitor(Component parentComponent,
								   Object message,
								   String note,
								   int min,
								   int max) {
	...
	private EnhancedProgressMonitor(Component parentComponent,
									Object message,
									String note,
									int min,
									int max,
									EnhancedProgressMonitor group) {
		this.min = min;
		this.max = max;
	...
	private class ProgressOptionPane extends JOptionPane {
		ProgressOptionPane(Object messageList) {
			super(messageList,
					...
					null,
					EnhancedProgressMonitor.this.cancelOption,
					null);
		}
		...
		public JDialog createDialog(final Component parentComponent, String title) {
			...
			if (parentComponent == null) {
				...
				window = getWindowForComponent(parentComponent);
			}
			////// nnwl END
...
//            }
					...
					if (dialog.isVisible() &&
						...
						if (isModal) {
							if (parentComponent instanceof JDialog) {
								((JDialog) parentComponent).setEnabled(true);
							} else if (parentComponent instanceof JFrame) {
								((JFrame) parentComponent).setEnabled(true);
							}
						}
						////// nnwl END
		...
		public AccessibleContext getAccessibleContext() {
			return EnhancedProgressMonitor.this.getAccessibleContext();
		}
	...
	public void setProgress(int nv) {
		...
		////// nnwl BEGIN
		if (nv >= max && !forceDialog && this.autoClose) {
			////// nnwl END
			close();
		} else {
			if (myBar != null) {
				...
				////// nnwl BEGIN
				boolean createDialogNow = false;
				if (this.isIndeterminate) {
					createDialogNow = true;
				}
				////// nnwl END
				else {
				...
				if (createDialogNow) {
					...
					if (this.isModal) {
						if (parentComponent instanceof JDialog) {
							((JDialog) parentComponent).setEnabled(false);
						} else if (parentComponent instanceof JFrame) {
							((JFrame) parentComponent).setEnabled(false);
						}
					}
				}
				////// nnwl END
	 ...
	 * progress message. Used, for example, to show which path the
	 * is currently being copied during a multiple-path copy.
	 *
